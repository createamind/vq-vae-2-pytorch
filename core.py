...
def cnn_layer(x):
    ...
    x = tf.nn.relu(tf.layers.conv2d(x, 16, [3, 3], strides=(2, 2), padding='SAME',
                                    kernel_initializer=tf.contrib.layers.xavier_initializer()))
    x = tf.nn.relu(tf.layers.conv2d(x, 32, [3, 3], strides=(2, 2), padding='SAME',
                                    kernel_initializer=tf.contrib.layers.xavier_initializer()))
    x = tf.nn.relu(tf.layers.conv2d(x, 128, [4, 4], strides=(1, 1), padding='VALID',
                                    kernel_initializer=tf.contrib.layers.xavier_initializer()))
    x = tf.layers.flatten(x)
    return tf.layers.dense(x, 128)
